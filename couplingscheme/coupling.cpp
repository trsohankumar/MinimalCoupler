...
namespace MinimalCoupler
{
    ...
    CouplingScheme::CouplingScheme()
        : _maxTime(0.0), _timeWindowSize(0.0), _currentTimeWindowNumber(0)
    {
    }
        ...
        int remoteSocket)
    {
        ...
        if (participantName == "Solid")
        {
            ...
            send(
                remoteSocket, 
                solidData.data(), 
                dispSize * sizeof(double), 
                0);
        }

        _currentTimeWindowNumber++;
        MINIMALCOUPLER_INFO("Time window: ", _currentTimeWindowNumber - 1 , " is complete. Moving to next time window. Start of next time window at: ", getCurrentTime());
